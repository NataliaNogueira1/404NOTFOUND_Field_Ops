...
export function InspectionReviewPage() {
  ...
  return (
    <div className="space-y-6">
                ...
                {answers.map((answer, index) => (
                  <div key={answer.id} className="rounded-fieldops border border-border bg-slate-50 p-4">
                    <div className="flex items-start justify-between gap-3">
                        ...
                        <p
                          className={
                            answer.nonConformityId
                              ...
                              : 'mt-2 text-sm font-semibold text-success-dark'
                          }
                        >
                          {answer.result}
                        </p>
                      ...
                      <button
                        className="focus-ring rounded-fieldops border border-border bg-white p-2 text-primary"
                        onClick={() => setPhoto(answer.evidence ?? null)}
                      >
                        <Camera size={18} />
                      </button>
                    </div>
      ...
      <Modal open={Boolean(photo)} title="Evidencia fotografica" onClose={() => setPhoto(null)}>
        {photo && (
          <div className="space-y-4">
            <div className="grid h-80 place-items-center rounded-card border border-border bg-gradient-to-br from-primary-light to-white text-primary-dark">
              <Camera size={64} />
            </div>
            <p className="text-sm text-muted">Arquivo: {photo} - Localizacao: -23.5015, -47.4526</p>
          </div>
        )}
      </Modal>
      <ConfirmDialog
