...
export function TemplateBuilderPage() {
  ...
  async function saveDraft(showToast = true) {
    ...
    try {
      if (/^\d+$/.test(id)) {
        await adminCatalogApi.updateTemplate(id, { title: title.trim(), category: category.trim(), description: description.trim() })
      }
      if (showToast) {
        setToast(true)
