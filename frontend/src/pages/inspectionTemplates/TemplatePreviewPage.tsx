...
export function TemplatePreviewPage() {
  ...
  const publicationIssues = useMemo(
    () => (template ? inspectionTemplatePendingIssues(template.title, template.category, template.sections) : []),
    [template],
  )
    ...
    <div className="space-y-6">
      <Card className="p-5">
        ...
        <div className="mt-3 flex flex-wrap gap-x-6 gap-y-2 text-sm text-muted">
          ...
          <p>
            Secoes: {sections.length} | Itens: {itemCount}
          </p>
        </div>
      </Card>
        ...
        <div className="overflow-hidden rounded-[22px] bg-app-bg">
          ...
          <div className="space-y-4 p-4">
            {sections.map((section, sectionIndex) => {
              const previousItemCount = sections
                .slice(0, sectionIndex)
                .reduce((total, previousSection) => total + previousSection.items.length, 0)
              return (
                <Card key={section.id} className="p-4 shadow-none">
                  <h3 className="mb-1 font-semibold">
                    Secao {sectionIndex + 1}: {section.title}
                  </h3>
                  {section.description && <p className="mb-3 text-xs text-muted">{section.description}</p>}
                  <div className="space-y-4">
                    {section.items.map((item, itemIndex) => (
                      <PreviewItem key={item.id} item={item} number={previousItemCount + itemIndex + 1} />
                    ))}
                  </div>
                </Card>
              )
      ...
      <div className="flex flex-wrap justify-between gap-3">
        <Link to={`/app/inspection-templates/${template.id}/edit`}>
        ...
        <Button
          disabled={template.status !== 'Rascunho' || publicationIssues.length > 0 || publishing}
          onClick={() => setConfirmPublish(true)}
        >
          {publishing ? 'Publicando...' : 'Publicar versao'}
        </Button>
      </div>
  ...
  return (
    <div className="border-t border-border pt-3 first:border-t-0 first:pt-0">
      ...
      <div className="mt-2 flex flex-wrap gap-2">
        {item.requireObservationOnFailure && (
          <span className="inline-flex items-center gap-1 text-xs font-medium text-warning-dark">
            <MessageSquare size={14} />
            Obs. na falha
          </span>
        )}
        {item.requireEvidenceOnFailure && (
          <span className="inline-flex items-center gap-1 text-xs font-medium text-danger-dark">
            <Camera size={14} />
            Evid. na falha
          </span>
        )}
...
function previewControl(item: TemplateItem): ReactNode {
  if (item.responseType === ResponseType.TEXT_SHORT)
    return <Input label="Resposta curta" id={`preview-${item.id}`} placeholder="Texto curto" disabled />
  if (item.responseType === ResponseType.TEXT_LONG)
    return <Textarea label="Resposta longa" id={`preview-${item.id}`} placeholder="Texto longo" disabled />
  if (item.responseType === ResponseType.NUMBER)
    return <Input label="Numero" id={`preview-${item.id}`} type="number" placeholder="0" disabled />
  if (item.responseType === ResponseType.BOOLEAN)
    return (
      <div className="grid grid-cols-2 gap-2">
        <Button variant="secondary" className="h-9 px-2 text-xs" disabled>
          Sim
        </Button>
        <Button variant="secondary" className="h-9 px-2 text-xs" disabled>
          Nao
  ...
  if (item.responseType === ResponseType.SINGLE_CHOICE)
    return (
      <Select label="Selecao unica" id={`preview-${item.id}`} disabled>
