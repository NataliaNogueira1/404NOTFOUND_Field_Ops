...
export function TemplatesPage() {
  ...
  const drafts = useSyncExternalStore(
    templateDraftStore.subscribe,
    templateDraftStore.snapshot,
    templateDraftStore.snapshot,
  )
  const fallbackRows = useMemo(
    () => [...drafts, ...templates.filter((template) => !drafts.some((draft) => draft.id === template.id))],
    [drafts],
  )
  ...
  const loadTemplates = useCallback(async () => {
    setLoading(true)
    try {
      const result = await adminCatalogApi.listTemplates({
        name: debouncedQuery,
        status,
        page: list.page,
        size: list.size,
  ...
  const columns: Column<TemplateSummary>[] = [
    {
      header: 'Titulo',
      sortKey: 'title',
      cell: (template) => <span className="font-medium">{template.title || 'Modelo sem titulo'}</span>,
    },
    ...
    { header: 'Itens', cell: (template) => template.itemCount },
    {
      ...
      cell: (template) => (
        <Badge tone={template.status === 'ACTIVE' ? 'success' : 'warning'}>
          {template.status === 'ACTIVE' ? 'Ativa' : 'Rascunho'}
        </Badge>
      ),
    },
    {
      ...
      cell: (template) => (
        <div className="flex flex-wrap gap-2">
          <Button
            variant="ghost"
            className="h-8 px-2"
            onClick={() => navigate(`/app/inspection-templates/${template.id}/edit`)}
          >
            <Pencil size={16} />
            Editar
          </Button>
          <Button
            variant="ghost"
            className="h-8 px-2"
            onClick={() => navigate(`/app/inspection-templates/${template.id}/preview`)}
          >
            <Eye size={16} />
            Previa
          </Button>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Modelos de inspecao"
        description="Crie e publique checklists utilizados pelos tecnicos."
        action={
...
function localTemplates(source: typeof templates, name: string, status: TemplateListStatus | '', sort: string) {
  ...
  const rows = source
    .filter(
      (template) =>
        `${template.title} ${template.category}`.toLowerCase().includes(query) &&
  ...
  return rows.sort(
    (left, right) =>
      String(left[key as keyof TemplateSummary] ?? '').localeCompare(
        String(right[key as keyof TemplateSummary] ?? ''),
      ) * (direction === 'desc' ? -1 : 1),
  )
}
