...
export function EquipmentPage() {
  ...
  async function changeStatus() {
    ...
    const next = changingStatus.status === EquipmentStatus.ACTIVE ? EquipmentStatus.INACTIVE : EquipmentStatus.ACTIVE
    try {
      await equipmentApi.updateStatus(changingStatus.id, next)
      setChangingStatus(null)
      showToast(
        next === EquipmentStatus.ACTIVE ? 'Equipamento ativado com sucesso.' : 'Equipamento inativado com sucesso.',
      )
      await loadEquipment()
    } catch (cause) {
  ...
  const columns: Column<ManagedEquipment>[] = [
    ...
    { header: 'Local', sortKey: 'site.name', cell: (item) => item.siteName },
    {
      ...
      sortKey: 'qrCode',
      cell: (item) => (
        <button className="inline-flex items-center gap-1 font-mono text-xs text-primary" onClick={() => setQr(item)}>
          <QrCode size={15} />
          {item.qrCode}
        </button>
      ),
    ...
    { header: 'Status', sortKey: 'status', cell: (item) => <EquipmentStatusBadge status={item.status} /> },
    {
      ...
      cell: (item) => (
        <div className="flex flex-wrap gap-2">
          ...
          <Button
            aria-label={`Ver QR Code de ${item.name}`}
            variant="ghost"
            className="h-8 px-2"
            onClick={() => setQr(item)}
          >
          ...
          <Button
            aria-label={`${item.status === EquipmentStatus.ACTIVE ? 'Inativar' : 'Ativar'} ${item.name}`}
            variant="ghost"
            className="h-8 px-2"
            onClick={() => setChangingStatus(item)}
          >
            <Power size={16} />
          </Button>
        </div>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={
          contextualSiteId
            ? 'Equipamentos vinculados ao local selecionado.'
            : 'Gerencie ativos, vinculos com locais e QR Codes unicos.'
        }
        action={
          <Button onClick={() => setEditing('new')} disabled={sites.length === 0 && !contextualSiteId}>
            <Plus size={17} />
            Novo equipamento
          </Button>
        }
      ...
      <Card className="grid gap-4 p-4 md:grid-cols-3">
        ...
        <Select
          ...
          id="equipment-site"
          value={siteId}
          disabled={Boolean(contextualSiteId)}
          onChange={(event) => list.update('siteId', event.target.value)}
        >
        ...
        >
          <option value="">Todos</option>
          <option value={EquipmentStatus.ACTIVE}>Ativo</option>
          <option value={EquipmentStatus.INACTIVE}>Inativo</option>
          <option value={EquipmentStatus.DECOMMISSIONED}>Descomissionado</option>
        </Select>
      </Card>
      {error && (
        <div
      ...
      <EquipmentModal
        ...
        target={editing}
        sites={sites}
        initialSiteId={contextualSiteId || siteId}
        lockSite={Boolean(contextualSiteId)}
        onClose={() => setEditing(null)}
        onSave={save}
      />
      <ConfirmDialog
        open={Boolean(changingStatus)}
        title={changingStatus?.status === EquipmentStatus.ACTIVE ? 'Inativar equipamento' : 'Ativar equipamento'}
        description={
          changingStatus?.status === EquipmentStatus.ACTIVE
            ? `O equipamento ${changingStatus?.name} deixara de aparecer em novas inspecoes.`
            : `O equipamento ${changingStatus?.name} voltara a aparecer em novas inspecoes.`
        }
        confirmLabel={changingStatus?.status === EquipmentStatus.ACTIVE ? 'Inativar' : 'Ativar'}
        variant={changingStatus?.status === EquipmentStatus.ACTIVE ? 'danger' : 'primary'}
...
function EquipmentModal({
  ...
  const [draft, setDraft] = useState<EquipmentInput>(() =>
    target && target !== 'new'
      ...
      : {
          siteId: initialSiteId || sites[0]?.id || '',
          name: '',
          assetNumber: '',
          serialNumber: '',
          manufacturer: '',
  ...
  const nameError = !draft.name.trim()
    ...
    : draft.name.trim().length > 200
      ...
      : ''
  const qrError = !draft.qrCode.trim()
    ? 'Informe ou gere um QR Code.'
    : draft.qrCode.trim().length > 100
      ? 'Use no maximo 100 caracteres.'
      : ''
  ...
  return (
    <Modal
      open
      title={target === 'new' ? 'Novo equipamento' : 'Editar equipamento'}
      onClose={onClose}
      footer={
        <>
    ...
    >
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        />
        <Input
          label="Nº Patrimonio (opcional)"
          id="equipment-form-asset"
          maxLength={50}
          value={draft.assetNumber}
          onChange={(event) => setDraft({ ...draft, assetNumber: event.target.value })}
        />
        <Input
          label="Nº Serie (opcional)"
          id="equipment-form-serial"
          maxLength={100}
        ...
        <Input
          ...
          id="equipment-form-manufacturer"
          maxLength={100}
        ...
        <div className="sm:col-span-2 grid grid-cols-[1fr_auto] items-end gap-2">
          <Input
            label="QR Code"
            id="equipment-form-qr"
            maxLength={100}
            value={draft.qrCode}
            error={qrError}
            onChange={(event) => setDraft({ ...draft, qrCode: event.target.value })}
          />
          <Button type="button" variant="secondary" onClick={() => setDraft({ ...draft, qrCode: generateQrCode() })}>
            <RefreshCw size={16} />
            Gerar QR Code
          </Button>
        </div>
        <Select
          ...
          value={draft.status}
          onChange={(event) => setDraft({ ...draft, status: event.target.value as EquipmentStatus })}
        >
          <option value={EquipmentStatus.ACTIVE}>Ativo</option>
          <option value={EquipmentStatus.INACTIVE}>Inativo</option>
          <option value={EquipmentStatus.DECOMMISSIONED}>Descomissionado</option>
        ...
        <Input
          label="Data de instalacao (opcional)"
          id="equipment-form-installed"
          type="date"
          value={draft.installedAt}
          onChange={(event) => setDraft({ ...draft, installedAt: event.target.value })}
        />
        {submitError && (
...
function EquipmentStatusBadge({ status }: { status: EquipmentStatus }) {
  const label =
    status === EquipmentStatus.ACTIVE ? 'Ativo' : status === EquipmentStatus.INACTIVE ? 'Inativo' : 'Descomissionado'
  const tone =
    status === EquipmentStatus.ACTIVE ? 'success' : status === EquipmentStatus.INACTIVE ? 'neutral' : 'danger'
  return <Badge tone={tone}>{label}</Badge>
}

function QrModal({ item, onClose }: { item: ManagedEquipment | null; onClose: () => void }) {
  return (
    <Modal open={Boolean(item)} title="QR Code do equipamento" onClose={onClose}>
      {item && (
        <div className="grid place-items-center gap-4">
          <div
            aria-label={`Representacao visual do QR Code ${item.qrCode}`}
            className="grid h-44 w-44 grid-cols-7 gap-1 rounded-card border border-border bg-white p-4"
          >
          ...
          <div className="text-center">
            ...
            <p className="font-mono text-sm text-muted">{item.qrCode}</p>
          </div>
        </div>
      )}
    </Modal>
