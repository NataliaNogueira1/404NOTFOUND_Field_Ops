...
interface BackendEquipment extends Omit<
  ManagedEquipment,
  | 'id'
  | 'siteId'
  | 'clientId'
...
export const equipmentApi = {
  ...
  async create(input: EquipmentInput) {
    return normalize(
      await apiRequest<BackendEquipment>('/api/v1/equipment', {
        method: 'POST',
  ...
  async update(id: string, input: EquipmentInput) {
    return normalize(
      await apiRequest<BackendEquipment>(`/api/v1/equipment/${id}`, {
        method: 'PUT',
        body: JSON.stringify(payload(input)),
      }),
    )
  ...
  async updateStatus(id: string, status: EquipmentStatus) {
    return normalize(
      await apiRequest<BackendEquipment>(`/api/v1/equipment/${id}/status`, {
        method: 'PATCH',
