...
export const Input = forwardRef<HTMLInputElement, InputProps>(({ className, error, id, label, ...props }, ref) => (
  <div className="space-y-1.5">
    ...
    <input
      id={id}
      ref={ref}
      className={cn('focus-ring h-11 w-full rounded-fieldops border bg-white px-3.5 text-sm text-text placeholder:text-slate-400 focus:border-primary', error ? 'border-danger' : 'border-border', className)}
