...
export function Badge({
  ...
  return (
    <span
      className={cn('inline-flex rounded-md px-2.5 py-1 text-xs font-semibold', tones[tone], className)}
      {...props}
    />
  )
}
...
const statusTone: Record<InspectionStatus, BadgeTone> = {
  DRAFT: 'neutral',
  ASSIGNED: 'primary',
  IN_PROGRESS: 'warning',
  SUBMITTED: 'primary',
...
export function SeverityBadge({ severity }: { severity: Severity }) {
  return <Badge tone={priorityTone[severity]}>{severityLabel[severity]}</Badge>
}
export function RoleBadge({ role }: { role: UserRole }) {
  return (
    <Badge tone={role === UserRole.ADMIN ? 'danger' : role === UserRole.SUPERVISOR ? 'primary' : 'success'}>
      {roleLabel[role]}
    </Badge>
  )
}
...
export function UserStatusBadge({ status }: { status: UserStatus }) {
  return <Badge tone={userStatusTone[status]}>{userStatusLabel[status]}</Badge>
}
