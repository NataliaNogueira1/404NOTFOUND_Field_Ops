...
| PBI-083 | *P1:* Dashboard com indicadores por estado e criticidade (API) | [#146](https://github.com/NataliaNogueira1/404NOTFOUND_Field_Ops/issues/146) | 🟡 Parcial — `GET /api/v1/dashboard/summary` agrega estados, prioridades e atrasos em três queries (sem N+1), aceita `from`, `to`, `client` e `technicianId`, e restringe acesso a ADMINISTRATOR/SUPERVISOR. `openNonConformities` retorna `0` até o domínio de não conformidades existir no backend. |
...
**Porcentagem: ~57%** concluído (8/14; 1 parcial: PBI-083 Dashboard).
